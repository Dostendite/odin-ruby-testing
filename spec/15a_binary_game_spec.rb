...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        fumo = 'ᗜˬᗜ'
        valid_input = '8'
        allow(game_input).to receive(:gets).and_return(letter, fumo, valid_input)
      end
  ...
  # Note: #verify_input will only return a number if it is between?(min, max)
    ...
    subject(:game_loop) { described_class.new(1, 10) }
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        min = game_loop.instance_variable_get(:@minimum)
        max = game_loop.instance_variable_get(:@maximum)
        valid_input = '5'.to_i
        result = game_loop.verify_input(min, max, valid_input)
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        max = game_loop.instance_variable_get(:@maximum)
        invalid_input = 17
        result = game_loop.verify_input(min, max, invalid_input)
